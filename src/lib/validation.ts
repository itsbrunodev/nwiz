...
function validateEndDeviceConfig(device: EndDevice): ValidationResult[] {
  const results: ValidationResult[] = [];
  ...
  if (
    ...
    subnetMask &&
    defaultGateway &&
    results.length === 0 // only check subnet if formats are valid
  ) {
  ...
  for (const device of network.devices) {
    switch (device.deviceType) {
      case "Router":
        ...
        break;
      case "Switch":
        results.push(...validateSwitchConfig(device, globallyDefinedVlans));
        break;
      case "PC":
      case "Server":
      case "Laptop":
        results.push(...validateEndDeviceConfig(device));
        break;
    }
  }
